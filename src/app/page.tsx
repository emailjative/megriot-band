    ...
    <div className="min-h-screen bg-transparent text-white">
      <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-sm border-b border-white/10 bg-black/80">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-4 md:px-6 py-3 md:py-4">
          <h1 className="text-base md:text-lg font-semibold tracking-tight uppercase text-white">Meg Riot</h1>
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            {navLabels.map((label, i) => (
              <a key={navHrefs[i]} href={navHrefs[i]} className="text-white/60 hover:text-white transition-colors">{label}</a>
            ))}
      ...
      <section className="section-overlay relative min-h-[80vh] md:h-screen flex flex-col items-center justify-center px-4 pt-16">
        <div className="relative z-10 text-center px-4">
          <p className="uppercase mb-4 md:mb-6 text-white/60" style={{ fontSize: 'clamp(0.9rem, 3vw, 1.2rem)', letterSpacing: '0.2em' }}>
          ...
          <p className="text-base md:text-xl max-w-md mx-auto mt-6 md:mt-8 text-white/70">
            {t.heroTagline}
          </p>
        </div>
      ...
      <section id="band" className="section-overlay relative py-20 md:py-32 px-4 md:px-6">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs md:text-sm uppercase tracking-[0.3em] mb-3 md:mb-4 text-white/50">{t.bandLabel}</p>
          <h3 className="text-4xl md:text-6xl font-bold tracking-tight mb-8 md:mb-12 text-white">{t.bandTitle}</h3>
          <p className="text-base md:text-xl leading-relaxed max-w-2xl mx-auto mb-10 md:mb-16 text-white/70">
            {t.bandText}
          </p>
      ...
      <section id="media" className="section-overlay relative py-20 md:py-32 px-4 md:px-6 border-t border-white/10">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs md:text-sm uppercase tracking-[0.3em] mb-3 md:mb-4 text-white/50">{t.mediaLabel}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
            <a href="https://instagram.com/megriotband" target="_blank" rel="noopener noreferrer"
              ...
              <p className="text-sm text-white/50">@megriotband</p>
              <p className="text-xl font-bold mt-1 text-white">Instagram</p>
              <p className="text-xs mt-3 text-white/40">Follow for behind the scenes</p>
            </a>
            <a href="https://www.youtube.com/@MegRiot" target="_blank" rel="noopener noreferrer"
              ...
              <p className="text-sm text-white/50">@MegRiot</p>
              <p className="text-xl font-bold mt-1 text-white">YouTube</p>
              <p className="text-xs mt-3 text-white/40">Live performances &amp; videos</p>
            </a>
            <a href="https://www.tiktok.com/@meg.riot" target="_blank" rel="noopener noreferrer"
              ...
              <p className="text-sm text-white/50">@meg.riot</p>
              <p className="text-xl font-bold mt-1 text-white">TikTok</p>
              <p className="text-xs mt-3 text-white/40">Short clips &amp; chaos</p>
            </a>
          </div>
        </div>
      </section>
      ...
      <section id="tour" className="section-overlay relative py-20 md:py-32 px-4 md:px-6 border-t border-white/10">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs md:text-sm uppercase tracking-[0.3em] mb-3 md:mb-4 text-white/50">{t.tourLabel}</p>
          ...
          <ul className="space-y-4 md:space-y-6 inline-block">
            <li>
              <div className="inline-flex items-center gap-4 p-5 md:p-8 rounded-2xl border bg-white/5 border-white/10">
                <div>
                  <p className="text-lg md:text-2xl font-semibold text-white">TBD</p>
                </div>
              </div>
            </li>
          </ul>
      ...
      <section id="merch" className="section-overlay relative py-20 md:py-32 px-4 md:px-6 border-t border-white/10">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs md:text-sm uppercase tracking-[0.3em] mb-3 md:mb-4 text-white/50">{t.merchLabel}</p>
          ...
          <div className="flex justify-center">
            <div className="rounded-2xl md:rounded-3xl border overflow-hidden max-w-md w-full bg-white/5 border-white/10">
              ...
              <div className="p-5 md:p-8">
                <p className="text-xl md:text-2xl font-semibold mb-2 text-white">{t.merchItem}</p>
                <a href="https://instagram.com/megriotband" target="_blank" rel="noopener noreferrer"
                  className="block md:inline-block px-6 py-3 rounded-full font-semibold text-sm text-center transition-colors bg-white text-black hover:bg-white/80">
                  {t.merchOrder}
                </a>
              </div>
            </div>
          </div>
          <p className="text-xl md:text-2xl mt-8 md:mt-12 text-center text-white/70">15 &euro;</p>
        </div>
      </section>

      <section id="contact" className="section-overlay relative py-20 md:py-32 px-4 md:px-6 border-t border-white/10">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs md:text-sm uppercase tracking-[0.3em] mb-3 md:mb-4 text-white/50">{t.contactLabel}</p>
          <h3 className="text-4xl md:text-6xl font-bold tracking-tight mb-8 md:mb-12 text-white">{t.contactTitle}</h3>
          <p className="text-base md:text-xl leading-relaxed mb-8 md:mb-12 max-w-2xl mx-auto text-white/70">
            {t.contactText}
          </p>
          <a href="https://instagram.com/megriotband" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-4 p-5 md:p-8 rounded-2xl border transition-colors bg-white/5 border-white/10 hover:border-white/20">
            <InstagramIcon color="white" />
            <span className="text-lg md:text-2xl font-semibold text-white">{t.contactDM}</span>
          </a>
        </div>
      </section>

      <footer className="py-8 md:py-12 px-4 md:px-6 text-center text-xs md:text-sm text-white/60">
        <p>{t.footer}</p>
      </footer>
    </div>
  );
